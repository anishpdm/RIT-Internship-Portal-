...
async function updateAssignment(formData: FormData) {
  ...
  if (!id || !title) redirect(`/admin/assignments/${id}/edit?error=missing`);
  ...
  if (error) redirect(`/admin/assignments/${id}/edit?error=db`);
  ...
  return (
    <>
      <PageHeader
        ...
        actions={
          <Link href={`/admin/assignments/${params.id}`} className="btn btn-ghost">
