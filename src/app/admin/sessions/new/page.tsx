...
async function createSession(formData: FormData) {
  ...
  const session_type = String(formData.get('session_type') ?? 'live') as 'live' | 'recorded' | 'self_learning';
  ...
  if (!internship_id || !title) redirect('/admin/sessions/new?error=missing');
  ...
  if (error || !session) redirect('/admin/sessions/new?error=db');

  await logAudit({
    actor_id: me.userId, actor_role: me.profile.role,
    action: 'session.create', entity_type: 'session', entity_id: session.id,
...
export default async function NewSessionPage() {
  ...
  const [{ data: internships }, { data: allLevels }] = await Promise.all([
  ...
  // Group levels by internship_id for client-side filtering
  ...
  return (
    <>
      <PageHeader
        ...
        actions={<Link href="/admin/sessions" className="btn btn-ghost"><ArrowLeft size={16}/> Back</Link>}
      />

      <form action={createSession} className="card max-w-3xl space-y-5">
        {/* Dynamic internship → level selector */}
        <InternshipLevelSelect internships={internships ?? []} levelsByInternship={levelsByInternship}/>

        <div>
          <label className="field-label">Title</label>
          <input name="title" className="field" required placeholder="e.g. Linear Regression — Foundations & Intuition"/>
        </div>
        ...
        <div>
          ...
          <div className="grid sm:grid-cols-3 gap-3">
            {[
              ...
              { v: 'self_learning', t: 'Self-learning', d: 'Dwell + reflection note' },
            ].map((opt, i) => (
        ...
        <div className="flex justify-end gap-2 pt-2">
          <Link href="/admin/sessions" className="btn btn-ghost">Cancel</Link>
