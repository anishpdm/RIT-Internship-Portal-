...
async function updateSession(formData: FormData) {
  ...
  redirect(me.profile.role === 'mentor' ? `/mentor/sessions/${id}` : `/admin/sessions/${id}`);
}
...
export default async function EditSessionPage({
  ...
  return (
    <>
      <PageHeader
        eyebrow={`Admin / Edit session`}
        title={session.title}
        subtitle={`Internship: ${(session as any).internships?.title ?? '—'}`}
        actions={
          <Link href={`/admin/sessions/${params.id}`} className="btn btn-ghost">
